...
def import_labelled_csv(
    ...
    with database() as connection:
        ...
        # Persist imported Threshold Lab samples before the pooled connection
        # is returned. Without this commit, the API can report inserted rows
        # even though a subsequent status request cannot see them.
        connection.commit()
...
def clear_imported_samples():
    with database() as connection:
        ...
        # Persist the clear operation for the same reason as imports.
        connection.commit()
